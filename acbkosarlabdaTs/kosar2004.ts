...
function ObjektumFeltolto(feltoltendoElem: string[]): Eredmenyek[] {
        let beolvasottAdatok: Eredmenyek[] = [];
        for (let i: number = 0; i < feltoltendoElem.length; i++) {
            let daraboltAdatok: string[] = feltoltendoElem[i].split(";");
            let aktEredmeny: Eredmenyek = {
            ...
            beolvasottAdatok.push(aktEredmeny);
        }
        return beolvasottAdatok;
    }
    
    var kosarazasAdat: Eredmenyek[] = ObjektumFeltolto(eredmenyek);
    console.log(kosarazasAdat);

    //3. feladat:

function RealMadrid(vizsgaltObjektum: Eredmenyek[]): [number, number] {
    let hazai: number = 0;
    let idegen: number = 0;
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].hazai == "Real Madrid") {
            hazai++;
        }
        else if (vizsgaltObjektum[i].idegen == "Real Madrid") {
            idegen++;
        }
    }
    return [hazai, idegen];
...
function RealMadridKiir(eredmenyTuple: [number, number]): void {
    console.log("3. feladat: Real Madrid: Hazai: " + eredmenyTuple[0] + ", Idegen: " + eredmenyTuple[1]);
}

RealMadridKiir(RealMadrid(kosarazasAdat));

//4. feladat:

function Dontetlen(vizsgaltObjektum: Eredmenyek[]): boolean {
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].hazai_pont == vizsgaltObjektum[i].idegen_pont) {
            return true;
        }
    }
    return false;
...
function DontetlenKiir(eredmeny: boolean): void {
    if (eredmeny) {
...
DontetlenKiir(Dontetlen(kosarazasAdat));


//5. feladat:

function BarcelonaCsapat(vizsgaltObjektum: Eredmenyek[]): string {
    let nev: string = "";
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].hazai.match("Barcelona")) {
            nev = vizsgaltObjektum[i].hazai;
        }
    }
    return nev;
}

function BarcelonaCsapatKiir(nev: string): void {
    console.log("5. feladat: barcelonai csapat teljes neve: " + nev);
}
BarcelonaCsapatKiir(BarcelonaCsapat(kosarazasAdat));
...
function November2004(vizsgaltObjektum: Eredmenyek[]): November[] {
    let csapatInfo: November[] = [];
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        if (vizsgaltObjektum[i].idopont == "2004-11-21") {
            let ujCsapat: November = {
                hazai: vizsgaltObjektum[i].hazai,
                idegen: vizsgaltObjektum[i].idegen,
                hazai_pont: vizsgaltObjektum[i].hazai_pont,
                idegen_pont: vizsgaltObjektum[i].idegen_pont

            }
            csapatInfo.push(ujCsapat);
        }
    }
    return csapatInfo;
}

function November2004Kiir(kiirandoEredmenyek: November[]): void {
    console.log("6. feladat:")
    for (let i: number = 0; i < kiirandoEredmenyek.length; i++) {
        console.log(kiirandoEredmenyek[i].hazai + "-" + kiirandoEredmenyek[i].idegen + " (" + kiirandoEredmenyek[i].hazai_pont + ":" + kiirandoEredmenyek[i].idegen_pont + ")");
    }
}
November2004Kiir(November2004(kosarazasAdat));

//7. feladat:

interface Stadionok {
    StadionNeve: string,
    MerkozesekSzama: number
}

function Stadionok(vizsgaltObjektum: Eredmenyek[]): Stadionok[] {
    let statisztikaAdatok: Stadionok[] = [];
    for (let i: number = 0; i < vizsgaltObjektum.length; i++) {
        let szerepelE: boolean = false;
        let adottIndex: number = 0;
        for (let j: number = 0; j < statisztikaAdatok.length; j++) {
            if (vizsgaltObjektum[i].helyszin == statisztikaAdatok[j].StadionNeve) {
                szerepelE = true;
                adottIndex = j;
            }
        }
        if (!szerepelE) {
            let ujStadion: Stadionok = { StadionNeve: vizsgaltObjektum[i].helyszin, MerkozesekSzama: 1 };
            statisztikaAdatok.push(ujStadion);
        }
        else {
            statisztikaAdatok[adottIndex].MerkozesekSzama++;
        }
    }
    return statisztikaAdatok;
}

function StadionokKiir(eredmenyObjektum: Stadionok[]): void {
    console.log("7. feladat:")
    for (let i: number = 0; i < eredmenyObjektum.length; i++) {
        if (eredmenyObjektum[i].MerkozesekSzama > 20) {
            console.log(eredmenyObjektum[i].StadionNeve + ": " + eredmenyObjektum[i].MerkozesekSzama);
        }
    }
}

StadionokKiir(Stadionok(kosarazasAdat));